...
function find_address(hash, cb) {
  Address.findOne({ $or: [{a_id: hash}, {alias: hash}] }, function(err, address) {
    if(address) {
      return cb(address);
    } else {
      return cb();
...
function update_address(hash, alias,  txid, amount, type, cb) {
          ...
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              lib.syncLoop(vin.length, function (loop) {
                ...
                update_address(nvin[i].addresses, nvin[i].alias, txid, nvin[i].amount, 'vin', function(){
                  loop.next();
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, vout[t].alias, txid, vout[t].amount, 'vout', function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
