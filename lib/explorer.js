...
module.exports = {
  ...
  get_hashrate: function(cb) {
    ...
    if (settings.nethash == 'netmhashps') {
      ...
      var uri = base_url + 'getnetworkcyclesps';
          ...
          module.exports.is_unique(arr_vout, vout[i].scriptPubKey.addresses, function(unique, index) {
            if (unique == true) {
              ...
              module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
                arr_vout.push({addresses: vout[i].scriptPubKey.addresses, amount: amount_sat, alias: vout[i].scriptPubKey.aliases});
                loop.next();
              });
            } else {
              // already exists
          ...
          module.exports.syncLoop(tx.vout.length, function (loop) {
            ...
            if (tx.vout[i].n == input.vout) {
              ...
              if (tx.vout[i].scriptPubKey.addresses) {
                  //TODO: does this work?
                addresses.push({hash: tx.vout[i].scriptPubKey.addresses[0], amount:tx.vout[i].value, alias:tx.vout[i].scriptPubKey.aliases});
              }
                loop.break(true);
                loop.next();
              //});
          ...
          module.exports.is_unique(arr_vin, addresses[0].hash, function(unique, index) {
            if (unique == true) {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
                arr_vin.push({alias:tx.vin[i].alias, addresses:addresses[0].hash, amount:amount_sat});
                //console.log(tx.vin[i].alias);
                loop.next();
              });
            } else {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
